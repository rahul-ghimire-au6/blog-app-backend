...
export const fetchAllCommentByBlogId = async (req: Request, res: Response) => {
    try {
        ...
        const commentData = await commentModel.find({ blogId }, { _v: 0 }).populate([{"path":"userId", "model":"users" }]).lean();
...
export const addComment = async (req: Request, res: Response) => {
    try {
        ...
        commentData.userId = userData._id;
        commentData.blogId = blogId;
        const createComment = await commentData.create(commentData);
